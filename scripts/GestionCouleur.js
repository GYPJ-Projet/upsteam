class GestionCouleur {
    ...
    init = () => {
        ...
        for (let i = 0, l = this._elsBtnModifier.length; i < l; i++) {
            this._elsBtnModifier[i].addEventListener('click', (e) => {
                ...
                this.modifierCouleur(e.target.dataset.jsId);
            });
        }
    ...
    modifierCouleur = (id) => {
        ...
        if (xhr) {	
            ...
            xhr.open('GET', 'index.php?GestionDonnees_AJAX&action=afficherFormulaireCouleur&id=' + id + '&page=' + this.page);
